...
class FiveRandomLinesNode:
    ...
    def combine_random_lines(self, global_prompt, text_1, text_2, text_3, text_4, text_5, prepend_mode, seed):
        texts = [text_1, text_2, text_3, text_4, text_5]
        positive_lines = []
        negative_lines = []

        # Parse global prompt (static, no random selection)
        # Note: Seed is assigned to global but not used since it's static
        random.seed(seed)  # Set seed for global, though unused
        global_positive, global_negative = self._parse_double_prompt(global_prompt)

        # Process random text fields with incremental seeds
        for i, text in enumerate(texts):
            lines = [line.strip() for line in text.strip().split('\n') if line.strip()]
            if lines:
                # Set incremental seed for this text field
                random.seed(seed + (i + 1))
                # Randomly select a line (controlled by seed)
        ...
        # Reset random seed to avoid affecting other nodes
        random.seed()
